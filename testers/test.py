...
def getNewHouseBills(url):
    text = getDynamicUrlText(url)
    index = text.find("bill/119th-congress/house-bill")
    newBillNum = int((text[index+31:index+35]))
...
# getNewHouseBills("view-source:https://www.congress.gov/search?q=%7B%22source%22%3A%22legislation%22%2C%22congress%22%3A%22119%22%7D")
...
with open('csv/input.csv', 'w', newline='', encoding='utf-8') as csvfile:
    writer = csv.writer(csvfile, delimiter='|', quotechar='"', quoting=csv.QUOTE_MINIMAL)
